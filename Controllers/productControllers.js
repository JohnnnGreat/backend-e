...
//@desc Get Product By a Given Text
//@route GET api/product/:text
...
const getProductByText = async (req, res) => {
    ...
    try {
        ...
        // Loop Through Products to find product
        const mainProduct = products.filter(product => {
            return product.name.toLowerCase().includes(text.toLowerCase())
        })
        console.log(mainProduct.length);
        if (mainProduct.length === 0) {
            return res.status(200).json({message: "No Products Found", products, productFound: false})
        }

        res.status(200).json({message: "Products Found", products: mainProduct, productFound: true})
    } catch (e) {
